...
void Current_Loop_FOC(FOC_Controller_t* controller, float iq_ref, float dt) {
    ...
    controller->dq_voltage.q = PID_Compute(&controller->pid_current_q, error_q, dt);
    
    // 6. d轴电流PID控制 (id=0策略)
    float id_ref = controller->target_current_d;  // 通常为0
    float error_d = id_ref - controller->dq_current.d;
    controller->dq_voltage.d = PID_Compute(&controller->pid_current_d, error_d, dt);
...
float Velocity_Loop_Control(FOC_Controller_t* controller, float velocity_ref, float dt) {
    ...
    // 速度PID控制
    float iq_ref = PID_Compute(&controller->pid_velocity, velocity_error, dt);
...
int FOC_Init(FOC_Controller_t* controller, uint8_t motor_id) {
    ...
    // 初始化PID控制器
    // 电流环PID参数 (需要根据实际电机调整)
    PID_Init(&controller->pid_current_q, 3.0f, 300.0f, 0.0f, VOLTAGE_LIMIT);
    PID_Init(&controller->pid_current_d, 3.0f, 300.0f, 0.0f, VOLTAGE_LIMIT);
    
    // 速度环PID参数 (需要根据实际电机调整)
    PID_Init(&controller->pid_velocity, 0.5f, 10.0f, 0.0f, CURRENT_LIMIT);
...
void FOC_SetControlMode(FOC_Controller_t* controller, FOC_ControlMode_t mode) {
    ...
    if (mode != controller->control_mode) {
        PID_Reset(&controller->pid_current_q);
        PID_Reset(&controller->pid_current_d);
        PID_Reset(&controller->pid_velocity);
        controller->openloop_angle = 0.0f;
    }
